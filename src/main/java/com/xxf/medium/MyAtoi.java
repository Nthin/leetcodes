...
class MyAtoi {
    ...
    int myAtoi(String str) {
        ...
        if (str.length() == 1) {
            return startChar >= 48 && startChar <= 57 ? Integer.parseInt(str) : 0;
        }
        if (startChar != 43 && startChar != 45 && (startChar < 48 || startChar > 57)) {
            return 0;
        }
        ...
        while (endIndex < str.length() && str.charAt(endIndex) >= 48 && str.charAt(endIndex) <= 57) {
            endIndex++;
        }
